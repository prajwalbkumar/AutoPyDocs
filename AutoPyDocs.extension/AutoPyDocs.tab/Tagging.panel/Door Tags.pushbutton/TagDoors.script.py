...
# Step 1: Allow user to select desired door types
door_types_collector = [dt for dt in FilteredElementCollector(doc).OfClass(FamilySymbol).OfCategory(BuiltInCategory.OST_Doors)]
door_type_options = {
    dt.get_Parameter(BuiltInParameter.SYMBOL_NAME_PARAM).AsString(): dt
    for dt in door_types_collector
    if dt.get_Parameter(BuiltInParameter.SYMBOL_NAME_PARAM) is not None
}

selected_door_types = forms.SelectFromList.show(
    sorted(door_type_options.keys()),
    title="Select Door Types to Tag",
    multiselect=True
)
if not selected_door_types:
    forms.alert("No door types selected. Exiting script.", exitscript=True)

selected_door_type_ids = [door_type_options[name].Id for name in selected_door_types]
...
tag_families = FilteredElementCollector(doc).OfClass(FamilySymbol).OfCategory(BuiltInCategory.OST_DoorTags)
tag_family_options = {fs.Family.Name: fs for fs in tag_families if fs.IsActive}
if not tag_family_options:
    forms.alert("No door tag families found in the project.", exitscript=True)

selected_tag_family_name = forms.SelectFromList.show(
    sorted(tag_family_options.keys()),
    title="Select Door Tag Family",
    multiselect=False
)
...
try:
    ...
    if linked_instance:
        ...
        if documentation_file == "Documentation File":
            ...
            doors_in_view = [door for door in FilteredElementCollector(ar_doc).OfCategory(BuiltInCategory.OST_Doors).WhereElementIsNotElementType() if door.GetTypeId() in selected_door_type_ids]
    ...
    for view in selected_views:
        if linked_instance:
            doors_in_view = [door for door in FilteredElementCollector(ar_doc, geom_view.Id).OfCategory(BuiltInCategory.OST_Doors).WhereElementIsNotElementType() if door.GetTypeId() in selected_door_type_ids]
        else:
            doors_in_view = [door for door in FilteredElementCollector(doc, view.Id).OfCategory(BuiltInCategory.OST_Doors).WhereElementIsNotElementType() if door.GetTypeId() in selected_door_type_ids]

        for door in doors_in_view:
            if linked_instance:
                ref = Reference(door).CreateLinkReference(ar_instance)
            else:
                ref = Reference(door)
...
if doors_tagged:
    ...
    output.print_md("### Total Doors Tagged: **{}**".format(total_doors_tagged))
